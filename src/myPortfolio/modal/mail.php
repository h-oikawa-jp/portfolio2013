...
//サイトのトップページのURL
$site_top = $_SERVER['SERVER_NAME'];

// 管理者メールアドレス ※メールを受け取るメールアドレス(複数指定する場合は「,」で区切ってください)
$to = "yfu94812@qkn.sakura.ne.jp";
...
$Referer_check_domain = $_SERVER['SERVER_NAME'];
...
// ファイルのディレクトリ情報 ※ファイル名を変更した場合は必ずここも変更してください。
$file_name ="mail.php"; //このPHPファイルの名前
$file_dir ="modal/"; //modalBox呼び出し元からの相対ディレクトリ
$file_back ="contact.php"; //POST元のフォームが記述されたファイル
...
$BccMail = "cr0nu.sc0urge.4tune@gmail.com";
...
foreach($_POST as $key=>$val) {
  if($val == "confirm_submit") $sendmail = 1;
	if($key == $Email && $mail_check == 1){
	  if(!checkMail($val)){
          $errm .= "<p class=\"error_messe\">「".$key."」：メールアドレスの形式が正しくありません。</p>\n";
          $empty_flag = 1;
	  }else{
		  $post_mail = h($val);
	  }
...
if($esse == 1) {
    ...
    if($val == "confirm_submit") ;
      ...
      for($i=0; $i<=$length; $i++) {
        if($key == $eles[$i] && empty($val)) {
          $errm .= "<p class=\"error_messe\">「".$key."」：必須入力項目です。</p>\n";
          $empty_flag = 1;
        }
      }
    }
  ...
  for($i=0; $i<=$length; $i++) {
    if($eles[$i] != "confirm_ok") {
      $errm .= "<p class=\"error_messe\">「".$eles[$i]."」：未選択です。</p>\n";
      $eles[$i] = "confirm_ok";
      $empty_flag = 1;
    }
  }
...
<h3>入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h3><?php echo $errm; ?>
<br><br>
<form action="<?php echo $file_dir,$file_back; ?>" method="POST">
<?php
foreach($_POST as $key=>$val) {
	$out = '';
	if(is_array($val)){
...
?>
	<input class="openmodalbox btn btn-primary" type="button" value="戻る" />
</form>
</div>
<?php
		}else{
?>
<div align="center" class="alert alert-info">以下の内容で間違いがなければ、「送信する」ボタンを押してください。</div>
<div align="center" class="well">
<form id="sendMail" action="<?php echo $file_dir,$file_name; ?>" method="POST">
...
<div align="center">
	...
	<input class="openmodalbox btn" type="submit" value="送信する">
	<input class="openmodalbox btn btn-primary offset1" type="button" value="修正する" onclick="jQuery('#sendMail').attr('action','<?php echo $file_dir,$file_back; ?>')">
</div>
</form>
</div>
<?php } ?>
...
<?php if($remail == 1) { ?>
自動返信メールが届かない場合はもう一度メールアドレスをご確認下さい。<br>
返信先メールアドレス：[ <?php echo $reto; ?> ]<br><br><br>
<?php } ?>

<a class="closeModalBox btn btn-primary" href="javascript:void(0);">閉じる</a>
